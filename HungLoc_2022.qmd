...
```{r}
trait_ideal <- analysis_trait[-c(1, 10, 11, 12, 13, 16:19, 21:26)]
...
h2 <- c()
i <- 1
for (trait in trait_ideal[-c(11)]) {  #CMD1mon, germination_percent did nor fit for tani
  mo_spatial <- SpATS(
    response = trait,
...
master_data$BLUPs_dona
master_data$h2_dona
```
```{r}
master_data$BLUPs_tani
```

# Multienviromental analysis

```{r}
variables <- trait_ideal[-c(11, 12)]
h2_fa <- c()
data_MET <- my_dat
